"""
ml_enhanced/models/cnn_lstm.py
──────────────────────────────
CNN + LSTM hybrid for temporal sleep signal classification.

Architecture:
    1-D Conv blocks  → feature extraction
    Bidirectional LSTM → temporal context
    Dense head         → classification

Uses TensorFlow / Keras.  Falls back gracefully when the library
is absent.
"""
...
class CNNLSTMModel(BaseModel):
    """
    1-D CNN feature extractor → Bidirectional LSTM → Dense classifier.
    ...
    def _build(self):
        if not _HAS_TF:
            warnings.warn("TensorFlow not installed – CNN-LSTM unavailable.")
            return
        ...
        x = layers.Conv1D(self.hidden_dim, 3, padding="same", activation="relu")(x)
        x = layers.BatchNormalization()(x)
        ...
        # Bidirectional LSTM
        x = layers.Bidirectional(layers.LSTM(self.hidden_dim // 2,
