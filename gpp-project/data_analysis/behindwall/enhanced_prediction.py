...
def predict_with_waveforms(current_data):
    """
    Enhanced prediction function that includes waveform features
    """
    try:
        # Calculate features from current data
        hr_data = current_data['HeartRate_BPM'].astype(float)
        rr_data = current_data['RespirationRate_BPM'].astype(float)
        range_data = current_data['Range_m'].astype(float)
        
        # Original features
        features = [
            hr_data.mean(),  # Avg_HR_clean
        ...
        heart_waveforms = current_data['HeartWaveform'].values
        breath_waveforms = current_data['BreathWaveform'].values
        
        features.extend([
            np.std(heart_waveforms) if len(heart_waveforms) > 1 else 0,  # HeartWave_Std
            np.max(heart_waveforms) - np.min(heart_waveforms) if len(heart_waveforms) > 1 else 0,  # HeartWave_Range
            np.std(breath_waveforms) if len(breath_waveforms) > 1 else 0,  # BreathWave_Std
            np.max(breath_waveforms) - np.min(breath_waveforms) if len(breath_waveforms) > 1 else 0,  # BreathWave_Range
        ])
        
        # Load enhanced model and predict
        enhanced_model = joblib.load(ENHANCED_MODEL_FILE)
        prediction = float(enhanced_model.predict(np.array(features).reshape(1, -1))[0])
        
        return {
            'Predicted_HR': prediction,
            'Features_Used': len(features),
            'Model_Type': 'Enhanced with Waveforms'
        }
...
print(f"📝 Use predict_with_waveforms(current_data) for improved predictions")
