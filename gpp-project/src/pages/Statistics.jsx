...
export default function Statistics() {
  ...
  useEffect(() => {
    ...
    if (loggedInUser) {
      // loggedUser is stored as plain email string
      setCurrentUser(loggedInUser);
      console.log("[Statistics] currentUser set to:", loggedInUser);
    }
  }, []);
  ...
  useEffect(() => {
    // Only fetch if currentUser is known (or if userFilter is "all")
    if (userFilter === "current" && !currentUser) {
      return; // Wait for currentUser to be loaded
    }

    // Reset all data immediately to prevent stale data from previous mode/filter
    setRows([]);
    setOverviewStats(null);
    setBarData(null);
    setLineData(null);
    setPieData(null);
    setScatterHRvsSQI(null);
    setScatterHRvsRangeSD(null);
    setScatterHRvsRR(null);
    setScatterHRvsRange(null);
    setScatterFinalVsClean(null);
    setMergedCorr(null);
    setAnomalyStats({ ok: 0, not_ok: 0 });
    setAnomalies([]);
    setHistHeart(null);
    setHistResp(null);
    setHistRange(null);
    ...
    setHRSQIGroups(null);
    setHRStressMatrix(null);
    setHypothesisResults(null);

    // load everything with current mode/userFilter
    fetchRuns();
    fetchOverview();
  ...
  function buildQueryParams() {
    ...
    if (userFilter === "current" && currentUser) {
      ...
      console.log("[Statistics] buildQueryParams: user filter active, user=", currentUser);
    }
    console.log("[Statistics] Query params:", params.toString());
  ...
  async function fetchCalibrationData() {
    ...
    const res = await fetch(`${EDA_BACKEND_BASE}/eda/hypothesis/calibration_data?${query}`);
    setCalibrationData(await res.json());
  }

  async function fetchHRSQIGroups() {
    const query = buildQueryParams();
    const res = await fetch(`${EDA_BACKEND_BASE}/eda/hypothesis/hr_sqi_groups?${query}`);
    setHRSQIGroups(await res.json());
  }

  async function fetchHRStressMatrix() {
    const query = buildQueryParams();
    const res = await fetch(`${EDA_BACKEND_BASE}/eda/hypothesis/hr_stress_matrix?${query}`);
  ...
  async function fetchRuns() {
    ...
    try {
      const query = buildQueryParams();
      const json = await safeFetchJson(`${EDA_BACKEND_BASE}/eda/runs?${query}`);
      if (!json) {
        setRows([]);
        setLoading(false);
        return;
  ...
  async function fetchOverview() {
    const query = buildQueryParams();
    const json = await safeFetchJson(`${EDA_BACKEND_BASE}/eda/overview?${query}`);
    if (!json) {
      return;
    }
    // Merge so that prepareCharts stats and overview stats coexist
    setOverviewStats((prev) => ({ ...(prev || {}), ...json }));
  }

  async function fetchMergedCorrelation() {
    const query = buildQueryParams();
    const json = await safeFetchJson(`${EDA_BACKEND_BASE}/eda/correlation_merged?${query}`);
    if (!json) {
      setMergedCorr(null);
      return;
    }
  ...
  async function fetchAnomalies() {
    const query = buildQueryParams();
    const res = await fetch(`${EDA_BACKEND_BASE}/eda/anomalies?${query}`);
    const json = await res.json();
  ...
  async function fetchHistogram(feature, setter) {
    const query = buildQueryParams();
    const json = await safeFetchJson(`${EDA_BACKEND_BASE}/eda/histogram?feature=${encodeURIComponent(feature)}&bins=30&${query}`);
  ...
  async function fetchBoxplot(feature, setter) {
    const query = buildQueryParams();
    const json = await safeFetchJson(`${EDA_BACKEND_BASE}/eda/boxplot?feature=${encodeURIComponent(feature)}&${query}`);
    ...
    const lastThreeRuns = useMemo(() => {
      ...
      const getSortValue = (entry) => {
        ...
        if (entry?.Run !== undefined && entry?.Run !== null) {
          const runNum = Number(entry.Run);
          if (!isNaN(runNum)) return runNum;
        }
        return -Infinity;
      };
      scored.sort((a, b) => getSortValue(b) - getSortValue(a));
      return scored.slice(0, 3);
    }, [rows]);

    const runSummarySubtitle = userFilter === "current" ? "Showing your last 3 runs" : "Showing last 3 runs across all users";
  ...
  return (
    <div className="flex flex-col items-center w-screen min-h-screen bg-black px-6 md:px-12 lg:px-20 pt-20 text-gray-200">
      ...
      <div className="w-full max-w-7xl mb-8 flex flex-col md:flex-row gap-6 items-center justify-center">
        {/* Mode Selector */}
        <div className="flex flex-col items-start gap-2">
          <label className="text-sm font-semibold text-gray-300">Monitoring Mode</label>
          <div className="flex gap-3">
            <button
              onClick={() => setMode("normal")}
              className={`px-4 py-2 rounded-lg font-medium transition border ${
                mode === "normal"
                  ? "bg-[#1a1a1a] text-white border-gray-400"
                  : "bg-[#0f0f0f] text-gray-400 border-gray-700 hover:border-gray-500 hover:text-gray-300"
              }`}
            ...
            <button
              ...
              disabled
              className="px-4 py-2 rounded-lg font-medium bg-[#0f0f0f] text-gray-600 border border-gray-800 cursor-not-allowed"
              style={{ outline: 'none', boxShadow: 'none' }}
              title="Sleep monitoring coming soon"
            >
              Sleep (Coming Soon)
            </button>
          </div>
        ...
        {currentUser && (
          <div className="flex flex-col items-start gap-2">
            <label className="text-sm font-semibold text-gray-300">Data Scope</label>
            <div className="flex gap-3">
              <button
    ...
    {scatterHRvsRangeSD && scatterHRvsRangeSD.datasets?.[0]?.data?.length > 0 ? (
      ...
      <div className="w-full max-w-7xl mt-10">
        <h2 className="text-2xl font-semibold mb-2">
          Run Summaries (last 3 of {rows.length} total runs)
        </h2>
        <p className="text-sm text-gray-400 mb-4">{runSummarySubtitle}</p>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {lastThreeRuns.length > 0 ? (
            lastThreeRuns.map((r) => (
              <div key={r.Run ?? r.id ?? r.Timestamp} className="bg-[#0b0b0b] border border-gray-700 rounded-xl p-4 shadow-md">
                <h4 className="text-md text-gray-100 font-semibold mb-2">
                  Run {r.Run ?? "-"} — <span className="text-sm text-gray-400">{r.Timestamp || "-"}</span>
                </h4>
            ...
            <div className="col-span-1 sm:col-span-2 lg:col-span-3 bg-[#0b0b0b] border border-gray-700 rounded-xl p-4 shadow-md text-gray-400">
              No runs available for this selection.
            </div>
          )}
        </div>
      </div>
...
function parseTimestamp(value) {
  ...
  if (match) {
    ...
    return Date.UTC(Number(yyyy), Number(mm) - 1, Number(dd), Number(hh), Number(min), ss ? Number(ss) : 0);
  }
  return null;
}